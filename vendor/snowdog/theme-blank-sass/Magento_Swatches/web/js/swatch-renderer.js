    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _RenderSwatchOptions: function (config) {
			...
			config.options.sort(function(a, b){
				...
				var nameA=parseInt(a.label); 
				var nameB=parseInt(b.label);
        ...
        updateBaseImage: function (images, context, isProductViewExist) {
            ...
            if (isProductViewExist) {
				...
				/** Base Image on Category page change FIX **/ 
				...
				/** Base Image on Category page change FIX **/
				var productImagePhoto = context.find('.product-image-photo')[0];
				 
				if ($(productImagePhoto).length ) {
					
					if (!$(productImagePhoto).hasClass('swatch-option-loading')) {
						$(productImagePhoto).appendClass('swatch-option-loading'); 
					}
					if ($(productImagePhoto).hasClass('swatch-option-loading')) {
						$(productImagePhoto).removeClass('swatch-option-loading'); 
					}
					
					$(productImagePhoto).attr('src', justAnImage.img);
					$(productImagePhoto).fadeIn('slow');
